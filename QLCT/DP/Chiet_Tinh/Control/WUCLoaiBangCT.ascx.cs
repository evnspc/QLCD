...
public partial class Chiet_Tinh_Control_WUCLoaiBangCT : System.Web.UI.UserControl
{
    ...
    private void LoadThongTinLoai(string ml)
    {
        DataTable dt = DBClass.GetTable("select * from Loai_Bang_Chiet_Tinh where Ma_Loai = '" + ml.Trim() + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    private void LoadDMLoai()
    {
        DataTable dt = DBClass.GetTable("select * from Loai_Bang_Chiet_Tinh order by Ten_Loai asc");
    ...
    protected void MyGrid01_CellSelectionChanged(object sender, Infragistics.Web.UI.GridControls.SelectedCellEventArgs e)
    {
        ...
        {
            this.LoadThongTinLoai(e.CurrentSelectedCells[0].Row.Items[0].Text.Trim());
        }
    }
    ...
    protected void MyGrid01_RowSelectionChanged(object sender, Infragistics.Web.UI.GridControls.SelectedRowEventArgs e)
    {
        ...
        {
            this.LoadThongTinLoai(e.CurrentSelectedRows[0].Items[0].Text.Trim());
        }
    }
    ...
    {
        DataTable dt = DBClass.GetTable("select * from Loai_Bang_Chiet_Tinh where Ma_Loai = '" + this.WMaLoai.Text.Trim() + "'");
        if (dt.Rows.Count < 1)
        {
            ...
            dt.Rows.Add(dtr);
            if (DBClass.UpdateTable("select * from Loai_Bang_Chiet_Tinh where Ma_Loai = '" + this.WMaLoai.Text.Trim() + "'", dt) == true)
            {
