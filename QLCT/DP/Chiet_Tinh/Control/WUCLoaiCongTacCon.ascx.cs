...
public partial class Chiet_Tinh_Control_WUCLoaiChiPhi : System.Web.UI.UserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (this.IsPostBack == false)
        {
            ...
            this.LoadLoaiCongTacCha();
        }
    }

    private void LoadLoaiCongTac()
    {
        DataTable dt = DBClass.GetTable("select CTC.*, LCT.Ten_Loai as Ten_Loai_Cha from Cong_Tac_Con CTC, Loai_Cong_Tac LCT where CTC.Ma_Cong_Tac = LCT.Ma_Loai order by CTC.Ma_Cong_Tac asc");
        this.MyGrid01.DataSource = dt;
        this.MyGrid01.DataBind();
    }

    private void LoadLoaiCongTacCha()
    {
        DataTable dt = DBClass.GetTable("select * from Loai_Cong_Tac order by Ten_Loai asc");
        this.DDLLoaiCha.DataSource = dt;
        this.DDLLoaiCha.DataBind();
    }

    private void LoadThongTinLoaiCongTac(string ml)
    {
        DataTable dt = DBClass.GetTable("select * from Cong_Tac_Con where Ma_Loai = '" + ml.Trim() + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void MyGrid01_CellSelectionChanged(object sender, Infragistics.Web.UI.GridControls.SelectedCellEventArgs e)
    {
        ...
        {
            this.LoadThongTinLoaiCongTac(e.CurrentSelectedCells[0].Row.Items[0].Text.Trim());
        }
    }
    ...
    protected void MyGrid01_RowSelectionChanged(object sender, Infragistics.Web.UI.GridControls.SelectedRowEventArgs e)
    {
        ...
        {
            this.LoadThongTinLoaiCongTac(e.CurrentSelectedRows[0].Items[0].Text.Trim());
        }
    }

    protected void WIBThemMoi_Click(object sender, EventArgs e)
    {
        DataTable dt = DBClass.GetTable("select * from Cong_Tac_Con where Ma_Loai = '" + this.WMaLoai.Text.Trim() + "'");
        if (dt.Rows.Count < 1)
        {
            DataRow dtr = dt.NewRow();
            dtr["Ma_Loai"] = this.WMaLoai.Text.Trim();
            dtr["Ten_Loai"] = this.WTenLoai.Text.Trim();
            dtr["Ma_Cong_Tac"] = this.DDLLoaiCha.SelectedValue.Trim();
            dt.Rows.Add(dtr);
            if (DBClass.UpdateTable("select * from Cong_Tac_Con where Ma_Loai = '" + this.WMaLoai.Text.Trim() + "'", dt) == true)
            {
