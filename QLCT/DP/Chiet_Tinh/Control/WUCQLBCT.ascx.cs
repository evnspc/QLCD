...
public partial class Chiet_Tinh_Control_WUCChietTinh : System.Web.UI.UserControl
{
    ...
    protected void BTra_Click(object sender, EventArgs e)
    {
        ...
        {
            if (KiemTraSoVB(this.WTraSoVB.Text.Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim()))
            {
                if (this.KiemTraThucXuat(this.WTraSoVB.Text.Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim()) == false)
                {
                    try
                    {
                        ListBox lb = (ListBox)Session["Temp_BCT"];
                        if (KiemTraTonTai(this.WTraSoVB.Text.Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim(), lb) == false)
                        {
                            lb.Items.Add(new ListItem("A", this.WTraSoVB.Text.Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim()));
        ...
        else
        {
            ...
            while (i < msvb.Length)
            {
                if (KiemTraSoVB(msvb[i].Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim()))
                {
                    if (this.KiemTraThucXuat(msvb[i].Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim()) == false)
                    {
                        try
                        {
                            ListBox lb = (ListBox)Session["Temp_BCT"];
                            if (KiemTraTonTai(msvb[i].Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim(), lb) == false)
                            {
                                lb.Items.Add(new ListItem("A", msvb[i].Trim() + "-" + this.LayMaDonViTuTaiKhoan().Trim()));                                
                            }
                        }
                        catch
                        {
    ...
    protected void BCapNhat_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            Redirect(ResolveUrl("~/Chiet_Tinh/CRBaoCaoCP01.aspx"), "_blank", "location=1,status=1,scrollbars=1;menubar=1,resizable=1");
        }
        catch { }    
    }
