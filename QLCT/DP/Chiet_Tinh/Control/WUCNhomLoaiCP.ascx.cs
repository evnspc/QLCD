...
public partial class Chiet_Tinh_Control_WUCLoaiChiPhi : System.Web.UI.UserControl
{
    ...
    {
        DataTable dt = DBClass.GetTable("select * from Chung_Loai_Chi_Phi order by Ten_Loai asc");
    ...
    {
        DataTable dt = DBClass.GetTable("select * from Chung_Loai_Chi_Phi where Ma_Loai = '" + ml.Trim() + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    {
        DataTable dt = DBClass.GetTable("select * from Chung_Loai_Chi_Phi where Ma_Loai = '" + this.WMaLoai.Text.Trim() + "'");
        if (dt.Rows.Count < 1)
        {
            ...
            dt.Rows.Add(dtr);
            if (DBClass.UpdateTable("select * from Chung_Loai_Chi_Phi where Ma_Loai = '" + this.WMaLoai.Text.Trim() + "'", dt) == true)
            {
