    ...
    private void LapBaoCao()
    {
        try
        {
            if (Session["Nhan_Vien"].ToString().Trim().Length > 0)
            {
                ...
                dtr["Tieu_De"] = "BÁO CÁO TỔNG HỢP BẢNG CHIẾT TÍNH";
                ...
                if (this.DDLXuatKho.SelectedValue.Trim() == "0")
                {
                    dtr["Loai_CT"] = "( Các bảng chiết tính đã thực hiện )";
                }
                else
                {
                    if (this.DDLXuatKho.SelectedValue.Trim() == "1")
                    {
                        dtr["Loai_CT"] = "( Các bảng chiết tính chưa thực hiện )";
                    }
                    else
                    {
                        dtr["Loai_CT"] = "( Tất cả các bảng chiết tính đã phát hành )";
                    }
                }
                ...
                strsqlcp = "select dmcp.Ma_Chi_Phi, dmcp.Ten_Chi_Phi, dmcp.DVT, Sum(cp.So_Luong) as So_Luong from Nhan_Vien nv, Bang_Chiet_Tinh bct, DM_Chi_Phi dmcp, Chi_Phi cp where nv.Tai_Khoan = bct.Nguoi_Lap and bct.So_Van_Ban = cp.So_Van_Ban and cp.ID_Vat_Tu = dmcp.ID_Chi_Phi and bct.Ngay_Lap >= '" + tn.ToShortDateString().Trim() + "' and bct.Ngay_Lap <= '" + dn.ToShortDateString().Trim() + "' and nv.Ma_Don_Vi = '" + dt.Rows[0]["Ma_Don_Vi"].ToString().Trim() + "' and dmcp.Ma_Danh_Phap <> 'NULL'";
                strsqlbct = "select nv.Ma_Don_Vi, bct.Ngay_Lap, nv.Ho_Ten as Nguoi_Lap, bct.So_Van_Ban, kh.Ho_Ten as Khach_Hang from Bang_Chiet_Tinh bct, Khach_Hang kh, Nhan_Vien nv where bct.Nguoi_Lap = nv.Tai_Khoan and bct.Ma_KH = kh.Ma_KH and bct.Ngay_Lap >= '" + tn.ToShortDateString().Trim() + "' and bct.Ngay_Lap <= '" + dn.ToShortDateString().Trim() + "' and nv.Ma_Don_Vi = '" + dt.Rows[0]["Ma_Don_Vi"].ToString().Trim() + "'";
                ...
                if (this.DDLXuatKho.SelectedValue != "2")
                {
                    if (this.DDLXuatKho.SelectedValue == "1")
                    {
                        strsqlcp = strsqlcp + " and bct.Ngay_Thu_Tien IS NULL";
                        strsqlbct = strsqlbct + " and bct.Ngay_Thu_Tien IS NULL";
                    }
                    else
                    {
                        strsqlcp = strsqlcp + " and bct.Ngay_Thu_Tien IS NOT NULL";
                        strsqlbct = strsqlbct + " and bct.Ngay_Thu_Tien IS NOT NULL";
                    }
                }

                strsqlcp = strsqlcp + " group by dmcp.Ma_Chi_Phi, dmcp.Ten_Chi_Phi, dmcp.DVT";
