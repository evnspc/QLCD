...
public partial class Chiet_Tinh_Control_WUCChietTinh : System.Web.UI.UserControl
{
    ...
    protected void BXuatExcel_Click(object sender, EventArgs e)
    {
        try
        {
            this.Response.Redirect(ResolveUrl("~/Chiet_Tinh/ReportEx.aspx") + "?ID=" + this.WSoVanBan.Text.Trim());
        }
        catch
        { }
    }
    ...
    protected void BThemVTTheoMau_Click(object sender, EventArgs e)
    {
        this.WebDialogWindow3.WindowState = Infragistics.Web.UI.LayoutControls.DialogWindowState.Normal;
    }

    protected void BThemTheoMau_Click(object sender, EventArgs e)
    {
        if (this.DDLLoaiCTMau.SelectedIndex > -1)
        {
            string id_vt;
            int i = 0;
            Boolean cokq = false;
            DataTable checkvt = DBClass.GetTable("select * from DM_Chi_Phi where Ma_Loai = '1'");
            DataTable dtvttheomau = DBClass.GetTable("select * from Chiet_Tinh_Mau where Ma_Loai = '" + this.DDLLoaiCTMau.SelectedValue.Trim() + "'");
            while (i < dtvttheomau.Rows.Count)
            {
                id_vt = dtvttheomau.Rows[i]["Ma_Chi_Phi"].ToString().Trim();
                DataTable dtvt = DBClass.GetTable("select * from Chi_Phi where So_Van_Ban = '" + this.WSoVanBan.Text.Trim() + "' and Ma_Chi_Phi = '" + id_vt.Trim() + "'");
                if (dtvt.Rows.Count < 1)
                {
                    DataRow[] mdtr = checkvt.Select("Ma_Chi_Phi = '" + id_vt.Trim() + "'");
    ...
    protected void BThoatTheoMau_Click(object sender, EventArgs e)
    {
        this.WebDialogWindow3.WindowState = Infragistics.Web.UI.LayoutControls.DialogWindowState.Hidden;
    }
    ...
    private void LoadLoaiCTMau()
    {
        ...
        this.DDLLoaiCTMau.DataSource = dt;
        this.DDLLoaiCTMau.DataBind();
    }
}
