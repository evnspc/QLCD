    ...
    private void LapBaoCao()
    {
       ...
       // {
            if (Session["Nhan_Vien"].ToString().Trim().Length > 0)
            {
                ...
                strsqlcp = "select dmcp.Ma_Danh_Phap as Ma_Chi_Phi, dmcp.Ten_Chi_Phi, dmcp.DVT, Sum(cp.So_Luong) as So_Luong from Nhan_Vien nv, Bang_Chiet_Tinh bct, DM_Chi_Phi dmcp, Chi_Phi cp where nv.Tai_Khoan = bct.Nguoi_Lap and bct.So_Van_Ban = cp.So_Van_Ban and cp.Ma_Chi_Phi = dmcp.Ma_Chi_Phi and bct.Ngay_Thu_Tien >= '" + tn.ToShortDateString().Trim() + "' and bct.Ngay_Thu_Tien <= '" + dn.ToShortDateString().Trim() + "' and nv.Ma_Don_Vi = '" + dt.Rows[0]["Ma_Don_Vi"].ToString().Trim() + "' and cp.Ma_Loai = '1'";
                ...
                while (dtrrbct.Read())
                {
                    ...
                    drtbct["So_Van_Ban"] = dtrrbct["So_Van_Ban"].ToString().Trim().Substring(0, dtrrbct["So_Van_Ban"].ToString().Trim().LastIndexOf("-")).Replace("-", "/CT-").Replace("D", "Đ");
                ...
                string cud = DateTime.Now.Day.ToString().Trim() + "-" + DateTime.Now.Year.ToString().Trim() + "-" + DateTime.Now.Year.ToString().Trim();
                //this.CRV1.ReportSource = this.CRSND;
                this.CRSND.ReportDocument.SetDataSource((DataSet)ds);
                //this.CRV1.RefreshReport();
                this.CRSND.ReportDocument.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, cud);
            }
        //}
        //catch
        //{
