...
public partial class Chiet_Tinh_Control_WUCReport : System.Web.UI.UserControl
{
    ...
    private void LapBangChietTinh(string svb)
    {
        ...
        " kh.Ho_Ten, kh.Ma_Tram, kh.Dia_Chi, lct.Ma_Loai as Ma_Cong_Tac, lct.Ten_Loai as Loai_Cong_Tac, bct.HS_Nhan_Cong, bct.HS_Phi_Khac as Phi_Truc_Tiep_Khac, bct.HS_CP_Chung as Chi_Phi_Chung," +
        ...
        if (dtbct.Rows.Count > 0)
        {
            ...
            dtrbct["Ho_Ten"] = dtbct.Rows[0]["Ho_Ten"].ToString().Trim() + " - Mã vùng: " + dtbct.Rows[0]["Ma_Vung"].ToString().Trim() + " - Mã trạm: " + dtbct.Rows[0]["Ma_Tram"].ToString().Trim();
            ...
            while (dtcprd.Read())
            {
                ...
                if (dtcprd["Ma_Loai"].ToString().Trim()=="2")
                {
                    ...
                    double ttnc = Math.Round(double.Parse(dtrcp["Don_Gia_Nhan_Cong"].ToString().Trim()) * (double.Parse(dtrcp["So_Luong"].ToString().Trim()) + double.Parse(dtrcp["So_Luong_KH"].ToString().Trim())), MidpointRounding.AwayFromZero);
                    ...
                    double ttvt = Math.Round(double.Parse(dtrcp["Don_Gia_Vat_Tu"].ToString().Trim()) * double.Parse(dtrcp["So_Luong"].ToString().Trim()), MidpointRounding.AwayFromZero);
                    dtrcp["TT_Vat_Tu"] = ttvt;
                    dtrcp["Don_Gia_Nhan_Cong"] = 0;
                    dtrcp["TT_Nhan_Cong"] = 0;
                    dtvt = dtvt + ttvt;
                    dtrcp["Ma_Dinh_Muc"] = demi.ToString().Trim();
                    demi++;
                }
            ...
            double nc = Math.Round(dtnc * double.Parse(dtrbct["HS_Nhan_Cong"].ToString().Trim()), MidpointRounding.AwayFromZero);
            ...
            {
                double TT_T = Math.Round(double.Parse(dtrbct["Tong_Vat_Tu"].ToString().Trim()) + nc, MidpointRounding.AwayFromZero);
                double TT_CPC = Math.Round(double.Parse(dtrbct["Chi_Phi_Chung_1"].ToString().Trim()) / 100 * double.Parse(dtrbct["Chi_Phi_Chung_2"].ToString().Trim()) / 100 * nc, MidpointRounding.AwayFromZero);
                double TT_G = Math.Round(TT_T + TT_CPC, MidpointRounding.AwayFromZero);
                ConvertClass cc = new ConvertClass();
                dtrbct["Bang_Chu"] = "(Bằng chữ: " + cc.Convert(TT_G.ToString().Trim(),'.', " Chấm") + " đồng)";
                dtrbct["T"] = TT_T;
                dtrbct["VL"] = Math.Round(double.Parse(dtrbct["Tong_Vat_Tu"].ToString().Trim()), MidpointRounding.AwayFromZero);
                dtrbct["NC"] = nc;
                dtrbct["C"] = TT_CPC;
                dtrbct["G"] = TT_G;
            }
            ...
            {
                double TT = Math.Round((double.Parse(dtrbct["Tong_Vat_Tu"].ToString().Trim()) + nc) * double.Parse(dtrbct["Phi_Truc_Tiep_Khac"].ToString().Trim()) / 100, MidpointRounding.AwayFromZero);
                double TT_T = Math.Round(double.Parse(dtrbct["Tong_Vat_Tu"].ToString().Trim()) + nc + TT, MidpointRounding.AwayFromZero);
                double TT_C = Math.Round(double.Parse(dtrbct["Chi_Phi_Chung"].ToString().Trim()) / 100 * nc, MidpointRounding.AwayFromZero);

                double TL = Math.Round((TT_T + TT_C) * double.Parse(dtrbct["Thu_Nhap_Chiu_Thue"].ToString().Trim()) / 100, MidpointRounding.AwayFromZero);
                double TT_K = Math.Round((TT_T + TT_C + TL) * double.Parse(dtrbct["Khao_Sat_Chiet_Tinh"].ToString().Trim()) / 100, MidpointRounding.AwayFromZero);
                double TT_G = Math.Round(TT_T + TT_C + TL + TT_K, MidpointRounding.AwayFromZero);
                double VAT = Math.Round(TT_G * 10 / 100, MidpointRounding.AwayFromZero);
                double TT_Z = Math.Round(TT_G + VAT, MidpointRounding.AwayFromZero);
                ConvertClass cc = new ConvertClass();                
        ...
        if (dtbct.Rows[0]["Loai_Chiet_Tinh"].ToString().Trim() == "0")
        {
            ...
            this.CRSND.ReportDocument.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, this.Request.QueryString["ID"].ToString().Trim());
        }
        else
        {
            if (dtbct.Rows[0]["Loai_Chiet_Tinh"].ToString().Trim() == "1")
            {
                ...
                this.CRSKH.ReportDocument.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, this.Request.QueryString["ID"].ToString().Trim());
            }
        }
